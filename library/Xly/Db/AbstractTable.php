...
# mail: xiangchen0814@gmail.com
# Created Time: 2022.11.12
#########################################################################

abstract class AbstractTable {

    const MYSQL_EXECUTE_SUCCESS = '00000';
    ...
    protected function reset(){
        ...
        if (is_null($this->_error)) {
            $this->_error = $this->_connection->errorCode();
        }
    }
    ...
    protected function _buildTableDataAndValue($tableData){
        if (!is_array ($tableData)){
            return;
        }
        $isInsert = strpos ($this->_query, 'INSERT');
    ...
    protected function _buildTableBatchData($tableData){
        if (!is_array ($tableData)){
            return;
        }
        $isInsert = strpos ($this->_query, 'INSERT');
        ...
        if ($isInsert !== false) {
            $this->_query .= '(`' . implode('`, `', array_keys($tableData[0])) . '`)';
            $this->_query .= ' VALUES';
        }
    ...
    protected function _buildWhereValue (){
        if (empty ($this->_where)){
            return;
        }
        $this->_query .= ' WHERE ';
    ...
    abstract public function batchInsert($insertData);
